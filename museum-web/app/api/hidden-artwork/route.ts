// app/api/hidden-artwork/route.ts
import { NextResponse } from 'next/server';
import { GoogleGenerativeAI } from '@google/generative-ai';
import path from 'path';
import { promises as fs } from 'fs';
import { HIDDEN_ARTWORK_PROMPT } from '@/prompts/hiddenArtworkPrompt'; // 아까 만든 프롬프트
// 또는 위에서 말한 유틸로 뺐다면:
// import { findArtwork, getMimeType } from '@/lib/server/artworkConfig';
...
export async function POST(request: Request) {
  try {
    ...
    const {
      ...
      artworkId?: string;
      interpretation?: string; // 사용자가 입력한 해석
    } = body;

    if (!artworkId || !interpretation) {
      return NextResponse.json(
        { error: 'Missing artworkId or interpretation' },
        { status: 400 },
      );
    }
    ...
    const composedPrompt = [
      HIDDEN_ARTWORK_PROMPT,
      '',
      '--- 히든 작품 채점 입력 ---',
      `artworkId: ${artworkId}`,
      `artworkMetadata: ${artwork.metadata ?? 'N/A'}`,
      `visitorInterpretation: "${interpretation}"`,
      '',
      '위 정보를 바탕으로 위의 규칙을 따라 한국어로만 답변하세요.',
    ].join('\n');
    ...
    console.error('Error in Gemini hidden-artwork API:', error);
    return NextResponse.json(
      {
